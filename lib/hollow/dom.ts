...
export async function buildDOM(html: string, url: string): Promise<DOMResult> {
  ...
  // PerformanceObserver — missing in Happy DOM; needed by Next.js turbopack
  // runtime and other performance-tracking bundles. No-op: we don't care about
  // performance metrics, only DOM structure.
  if (!win.PerformanceObserver) {
    win.PerformanceObserver = class PerformanceObserver {
  ...
  // Promise.prototype.finally — Happy DOM's Promise implementation may omit it.
  // Stripe and other Next.js bundles call .finally() on promise chains.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const WinPromise = (win.Promise ?? Promise) as any;
  if (WinPromise && !WinPromise.prototype.finally) {
    WinPromise.prototype.finally = function <T>(fn: () => unknown): Promise<T> {
      return this.then(
        (val: T)  => Promise.resolve(fn()).then(() => val),
        (err: unknown) => Promise.resolve(fn()).then(() => { throw err; })
      );
    };
  }
